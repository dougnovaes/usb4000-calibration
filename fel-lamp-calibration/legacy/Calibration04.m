%% Comparacao de Modelos: Corpo Cinza vs Linear vs Tabelada (real) vs Hibrida
...
% Physica 20, 690) com a correcao de luz espalhada de Larrabee (1959,
% JOSA 49, 619).
...
% Interpolacao 2D (wavelength, temperature); 'linear' e o metodo
% justificado pelo proprio relatorio (interpolacao linear em T entre
...
%% 3. Definicao dos quatro modelos
...
% Modelo 4: hibrido - tabela real x correcao residual linear pequena
...
fprintf('\n=== COMPARACAO DE MODELOS (emissividade real: NASA TN D-1088) ===\n');
...
plot(x_plot*1000, model1(p1,x_plot), '-g','LineWidth',1.2);
plot(x_plot*1000, model2(p2,x_plot), '-r','LineWidth',1.2);
plot(x_plot*1000, model3(p3,x_plot), '-m','LineWidth',1.2);
plot(x_plot*1000, model4(p4,x_plot), '-k','LineWidth',1.6);
...
legend('Dados', sprintf('Cinza (T=%.0fK)',p1(2)), sprintf('Linear (T=%.0fK)',p2(2)), ...
    sprintf('Tabelada real (T=%.0fK)',p3(2)), sprintf('Hibrida (T=%.0fK)',p4(2)), 'Location','best');
title('Comparacao dos Quatro Modelos'); grid on;
...
plot(x_um*1000, r4pct, '-ok','MarkerSize',4,'LineWidth',1.3);
yline(0,'--k');
xlabel('Comprimento de Onda (nm)'); ylabel('Erro Residual (%)');
legend('Tabelada real (fisica pura)','Hibrida (fisica + residual)','Location','best');
title('Residuos: Tabelada vs Hibrida'); grid on;